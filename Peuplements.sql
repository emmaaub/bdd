...
BEGIN
    ...
    if v_nb_analyse_hors_norme >= 3 THEN 
        UPDATE Sang_Analyse 
        SET Date_Prochaine_Analyse_Sang = v_local_date_analyse + INTERVAL '5' HOUR
        WHERE Date_Analyse_Sang = SYSDATE AND Id_Patient = p_id_patient;
    else
        UPDATE Sang_Analyse 
        SET Date_Prochaine_Analyse_Sang = v_local_date_analyse + INTERVAL '6' DAY
        WHERE Date_Analyse_Sang = SYSDATE AND Id_Patient = p_id_patient;
    end if;
...
CALL Peuplement_Analyse_Sang (SYSTIMESTAMP, 0, 1, 0, 1, 1, 0, 8);
